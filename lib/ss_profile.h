...
struct ss_profile {
	...
	uint8_t A001[A001_LEN];          /* 32 bytes KI, 32 bytes OPC, 2 byte indication encoded as HEX */
	uint8_t A004[A004_LEN];          /* TODO determine precise encoding scheme used. Contains multiple records in theory. */
	uint8_t SMSP[SMSP_RECORD_SIZE];  /* SMS related parameters. */
};

/**
 * @brief Decode an Onomondo SoftSIM profile
 *
