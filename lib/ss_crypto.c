...
int ss_utils_setup_key(size_t key_len, uint8_t key[static key_len], enum key_identifier_base key_id)
{
	...
	/* Check if the PSA crypto library was initialized successfully */
	if (status != PSA_SUCCESS) {
		LOG_ERR("Failed to initialize PSA crypto library. %d", status);
		return -1;
	} else {
		LOG_DBG("PSA crypto library initialized successfully.");
	}

	switch (key_id) {
	case KEY_ID_KI:
		/* KI only used for encryption */
		ss_utils_setup_key_helper(key_len, key, key_id, PSA_KEY_USAGE_ENCRYPT,
					  PSA_ALG_CBC_NO_PADDING, PSA_KEY_TYPE_AES);
		break;
	case KEY_ID_KIC:
		/* KIC only used for encryption/decrypt AES operations */
		ss_utils_setup_key_helper(key_len, key, key_id,
					  PSA_KEY_USAGE_ENCRYPT | PSA_KEY_USAGE_DECRYPT,
					  PSA_ALG_CBC_NO_PADDING, PSA_KEY_TYPE_AES);
		break;
	case KEY_ID_KID:
		/* KID only used for CMAC operations */
		ss_utils_setup_key_helper(key_len, key, key_id, PSA_KEY_USAGE_SIGN_MESSAGE,
	...
	LOG_DBG("Key %d setup successfully", key_id);
